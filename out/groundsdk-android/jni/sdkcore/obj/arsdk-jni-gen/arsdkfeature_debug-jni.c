...
Java_com_parrot_drone_sdkcore_arsdk_ArsdkFeatureDebug_nativeClassInit(JNIEnv *env, jclass jcls) {
	...
	s_cb_debug_cache.jmid_tag_notify = (*env)->GetStaticMethodID(env, jcls, "tagNotify", "(Lcom/parrot/drone/sdkcore/arsdk/ArsdkFeatureDebug$Callback;Ljava/lang/String;)V");
}
...
static int evt_debug_tag_notify(JNIEnv *env, jclass clazz, struct arsdk_cmd *cmd, jobject callback) {
	const char * id;
	int res = arsdk_cmd_dec_Debug_Tag_notify(cmd, &id);
	if (res < 0)
		return res;
	(*env)->CallStaticVoidMethod(env, clazz, s_cb_debug_cache.jmid_tag_notify, callback, (*env)->NewStringUTF(env, id));
	return 0;
}
...
Java_com_parrot_drone_sdkcore_arsdk_ArsdkFeatureDebug_nativeDecode(JNIEnv *env, jclass jcls, jlong nativeCmd, jobject callback) {
	...
	switch(cmd->cmd_id) {
		...
		case 5: return evt_debug_tag_notify(env, jcls, cmd, callback);
	}
	return -1;
}
...
JNIEXPORT jint JNICALL
Java_com_parrot_drone_sdkcore_arsdk_ArsdkFeatureDebug_nativeEncodeTag(JNIEnv *env, jclass jcls, jlong nativeCmd, jstring value) {
	struct arsdk_cmd *cmd = (struct arsdk_cmd *)(uintptr_t)nativeCmd;
	const char* c_value = (*env)->GetStringUTFChars(env, value, NULL);
	int res = arsdk_cmd_enc_Debug_Tag(cmd, c_value);
	if (c_value != NULL) (*env)->ReleaseStringUTFChars(env, value, c_value);
	return res;
}
