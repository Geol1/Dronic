...
Java_com_parrot_drone_sdkcore_arsdk_ArsdkFeatureDri_nativeClassInit(JNIEnv *env, jclass jcls) {
	...
	s_cb_dri_cache.jmid_dri_type = (*env)->GetStaticMethodID(env, jcls, "driType", "(Lcom/parrot/drone/sdkcore/arsdk/ArsdkFeatureDri$Callback;Ljava/lang/String;II)V");
}
...
static int evt_dri_dri_type(JNIEnv *env, jclass clazz, struct arsdk_cmd *cmd, jobject callback) {
	const char * id;
	int32_t type;
	int32_t status;
	int res = arsdk_cmd_dec_Dri_Dri_type(cmd, &id, &type, &status);
	if (res < 0)
		return res;
	(*env)->CallStaticVoidMethod(env, clazz, s_cb_dri_cache.jmid_dri_type, callback, (*env)->NewStringUTF(env, id), (jint)type, (jint)status);
	return 0;
}
...
Java_com_parrot_drone_sdkcore_arsdk_ArsdkFeatureDri_nativeDecode(JNIEnv *env, jclass jcls, jlong nativeCmd, jobject callback) {
	...
	switch(cmd->cmd_id) {
		...
		case 6: return evt_dri_dri_type(env, jcls, cmd, callback);
	}
	return -1;
}
...
JNIEXPORT jint JNICALL
Java_com_parrot_drone_sdkcore_arsdk_ArsdkFeatureDri_nativeEncodeSetDriType(JNIEnv *env, jclass jcls, jlong nativeCmd, jint type, jstring id) {
	struct arsdk_cmd *cmd = (struct arsdk_cmd *)(uintptr_t)nativeCmd;
	const char* c_id = (*env)->GetStringUTFChars(env, id, NULL);
	int res = arsdk_cmd_enc_Dri_Set_dri_type(cmd, type, c_id);
	if (c_id != NULL) (*env)->ReleaseStringUTFChars(env, id, c_id);
	return res;
}
